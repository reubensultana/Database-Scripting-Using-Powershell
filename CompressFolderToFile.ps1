param([String]$CompressFolder='C:\MSSQL\SCRIPTS', # the directory where you want to store them
	  [String]$BackupFolder='C:\MSSQL\SCRIPTS', # the destination folder for the zip file
	  [String]$ZipFileName='SCRIPTS_20130630.zip') # the destination zip file

# Zip functions adopted from http://www.technologytoolbox.com/blog/jjameson/archive/2012/02/28/zip-a-folder-using-powershell.aspx 
# Usage: .\CompressFolderToFile.ps1 -CompressFolder "C:\MSSQL\SCRIPTS" -BackupFolder "C:\MSSQL\SCRIPTS\" -ZipFileName "SCRIPTS_20130630.zip"
...
#        $zipFileName = $parentDir.FullName + "\" + $directory.Name + ".zip"
#    }
    If ($zipFolder.EndsWith("\") -eq $true) {
        $zipFileName = $zipFolder + $zipFileName
    }
    Else {
        $zipFileName = $zipFolder + "\" + $zipFileName
    }

	If ((Test-Path $zipFolder) -eq $false) {
    ...
    [int] $expectedCount = (Get-ChildItem $directory -Force -Recurse -Exclude $zipFileName).Count
    $expectedCount += 1 # account for the top-level folder
    
    $zipFile.CopyHere($directory.FullName)
	#Get-ChildItem $directory -Force -Recurse -Exclude $zipFileName | foreach {$zipFile.CopyHere($_.fullname)}
...
[IO.DirectoryInfo] $directory = Get-Item $CompressFolder
ZipFolder $directory $BackupFolder $ZipFileName
RETURN
